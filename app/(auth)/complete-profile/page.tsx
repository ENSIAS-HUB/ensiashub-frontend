...
const FILIERES = ['GL', 'GD', 'D2S', '2IA', '2SCL', 'SSE', 'CSCMC', 'IDF', 'BI&A'];
const ANNEES = [
...
function CompleteProfileForm() {
  ...
  // Token + user passed in query params from backend redirect
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Parse user, update filiere + annee, persist auth
      let user: User;
      try {
      ...
      const updatedUser: User = { ...user, filiere, annee };
      ...
      >
        ...
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold text-white">Complétez votre profil</h1>
            <p className="mt-1 text-sm text-slate-400">
              Sélectionnez votre filière et votre année pour rejoindre votre groupe.
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={filiere} onValueChange={setFiliere}>
                ...
                <SelectContent className="bg-slate-800 border-slate-700">
                  {FILIERES.map((f) => (
                    <SelectItem key={f} value={f} className="text-white focus:bg-slate-700">
                      {f}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={annee} onValueChange={setAnnee}>
                ...
                <SelectContent className="bg-slate-800 border-slate-700">
                  {ANNEES.map((a) => (
                    <SelectItem key={a.value} value={a.value} className="text-white focus:bg-slate-700">
                      {a.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            >
              {loading ? (
                ...
                <>Confirmer et accéder à ENSIAS Hub<ArrowRight className="size-4" /></>
              )}
            </Button>
          </form>
        </div>
